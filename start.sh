...
cd "$SCRIPT_DIR/backend" && source ../.venv/bin/activate && python -m uvicorn server:app --host 127.0.0.1 --port 8000 &
BACKEND_PID=$!

echo "[2/2] Launching Nuxt Dashboard (Port 3000)..."
cd "$SCRIPT_DIR/frontend" && NUXT_PUBLIC_API_BASE=http://localhost:8000 npm run dev &
FRONTEND_PID=$!
